...
<script setup lang="ts">
  ...
  function setUpConfirmArea({ data, boundRect }: { data: ImageData; boundRect: Box }) {
    ...
    setTimeout(() => {
      ...
      executeBoundSelected();
    }, 100);
  }
  defineExpose({ setUpConfirmArea });

  function executeBoundSelected() {
    ...
    if (selectVisible.topLeft && topCount === 1 && leftCount === 1) {
      // 上边、左边和左上角端点
    ...
    if (selectVisible.topRight && topCount === 1 && rightCount === 1) {
      // 上边、右边和右上角端点
      const topBoundPoints = points.filter((p) => p[1] === 0);
    ...
    if (selectVisible.bottomRight && bottomCount === 1 && rightCount === 1) {
      // 底边、右边和右下端点
    ...
    if (selectVisible.bottomLeft && bottomCount === 1 && leftCount === 1) {
      // 底边、左边和左下端点
      const bottomBoundPoints = points.filter((p) => p[1] === _boundRect[3] - 1);
    ...
    confirmCtxRef.value?.clearRect(0, 0, 600, 600);
